// here citiesAndState is array containing data of citiesAndState and states
...
function findMatches(wordToMatch, citiesAndState) {
  return citiesAndState.filter((place) => {
    // here we need to figure out if city or state entered by user matches with one present in the object citiesAndState
    const regex = new RegExp(wordToMatch, 'gi')
    // here g stands for
    // search globally
    // and i stand for
    // insensitivity which means it will consider uppercase and lowercase alphabets the same
...
function displayMatches() {
  const matchedArray = findMatches(this.value, citiesAndState)
  const html = matchedArray
    .map((place) => {
      ...
      return `
        <li>
            <span class="name">city: ${place.city}, state: ${place.state}</span>
            <br>
